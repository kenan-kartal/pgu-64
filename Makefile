...
build/readrec.o: 06-readrec.s linux-32.s record-def.s
	as $(DEBUG_ARGS) $(AS_ARGS_32) $(INC_ARGS) -o $@ $<
...
build/librecord.so: readrec.o writerec.o
	ld $(DEBUG_ARGS) $(LD_ARGS_32) -shared -o $@ $^

# Build executables-32
build/exit: exit.o
	ld $(DEBUG_ARGS) $(LD_ARGS_32) -o $@ $^
build/max: max.o
	ld $(DEBUG_ARGS) $(LD_ARGS_32) -o $@ $^
build/power: power.o
	ld $(DEBUG_ARGS) $(LD_ARGS_32) -o $@ $^
